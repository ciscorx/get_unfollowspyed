...
click_page () {
    ...
    DISPLAY=$DSP xte "mousemove 10 600" "usleep 50000" "mouseclick 1" "usleep 50000"
...
sleep 5
 ...
 var=$(DISPLAY=$DSP xclip -out -selection clipboard)
   # remove all lines up to and including the last chinese containing line
 
#var=$(echo "$var" | sed '/Look up details/,/Translation result/d')
#var=$(echo "$var" | sed '/Look up details$/,/Translation result$/d')

var=$(echo "$var" | sed '/\xe2\x80\x8bLook up details/,/Translation result$/d')

#var=$(echo "$var" | awk 'BEGIN {buf="";} /[\x{4E00}-\x{9FFF}]/ {buf=""; next;} {buf = buf $0 ORS;} END {printf "%s", buf;}')
#var=$(echo "$var" | awk '/[\x{4E00}-\x{9FFF}]/ {buf=""; next} {buf = buf $0 ORS} END {print buf}')

# We get the line number of the last Chinese line

last_chinese_line_num=$(echo "$var" | grep -n -E '[\x{4E00}-\x{9FFF}]' | tail -1 | cut -d: -f1)
last_chinese_line=$(echo "$var" | grep -nP "^\p{Han}" | cut -f1 -d: | tail -1)
var=$(echo "$var" | awk -v lastLine=$last_chinese_line 'NR > lastLine')

if [ -z "$last_chinese_line" ]; then
    echo "No Chinese lines found."
else
    var=$(echo "$var" | awk -v lastLine=$last_chinese_line_num 'NR > lastLine')
fi
